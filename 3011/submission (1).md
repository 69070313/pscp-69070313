...
```text
ผมเข้าใจว่าโจทย์ต้องการให้เรารับค่าinputมาเป็นstrโดยสีที่รับมาได้มีแค่แม่สี3สีแล้วให้นำมาผสมกันใน3สีหากสีที่inputมาเป็นแม่สี3สีจริงให้นำไปเข้าโปรแกรมต่อเพิ่อรับoutputเป็นการผสมสีหากสีที่ได้ไม่ใช่แม่สีให้ตอบerrorแทน
```
...
```text
1.ผมตั้งใจจะรับค่าinputที่ได้มาก่อน
2.เสร็จแล้วเปลี่ยนให้เป็นตัวพิมพ์ใหญ่ทั้งหมดก่อนเพื่อให้ง่ายต่อการเช็ค
3.สร้างกลุ่มเซ็ทมาเพื่อเก็บแต่ละสีไว้ให้เช็คทีหลัง
3.เช็คแต่ละเงื่อนไขว่าถ้าinput1เป็นสีนี้input2สีนี้outputจะเป็นยังไง
4.print outputออกมา
```
...
```text
วิธีสุดท้ายใช้ตามแผนแรกที่วางไว้แต่มารู้ทีหลังว่าไม่ต้องสร้างเซ็ทก็ได้เลยแก้เอาตรงเซ็ทออก
```
...
```text
เพราะมีตัวเล็กแทรกในคำทำให้ดูยาก
```
...
```text
เพราะสีนึงไม่อยู่ในแม่สีที่กำหนด
```
...
```text
ช่วยเช็คและใบ้ว่าเรายังขาดอะไรอยู่
```

คุณยังทำอะไรด้วยตนเอง

```text
ได้คิดและเช็คโค้ดด้วยตัวเองอีกรอบว่าจริงรึเปล่า
```
